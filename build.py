...
def share_row(page):
    ...
    Instagram is the exception: it has no web share endpoint at all, so its
    icon opens the shop's own profile rather than pretending to share. Copy
    link beside it is the thing a customer pastes into a story or a DM.
    """
    ...
    links = [
        ("Share on WhatsApp", "https://wa.me/?text=%s%%20%s" % (q(text), q(url)), "whatsapp"),
        ("Share on Facebook", "https://www.facebook.com/sharer/sharer.php?u=%s" % q(url), "facebook"),
        ("%s on Instagram" % config_value("storeName", "Cassaro Beauty"),
         config_value("instagramUrl", "https://www.instagram.com/cassaro_beauty"), "instagram"),
    ]
    out = []
